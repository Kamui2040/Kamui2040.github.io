(() => {
  ...
  const PLATFORMS = [
    { key: "github", hosts: ["github.com"], labels: ["github", "github releases"], actionLabel: "GitHub" },
    { key: "nexus", hosts: ["nexusmods.com"], labels: ["nexus mods"], actionLabel: "Nexus Mods" },
    { key: "kofi", hosts: ["ko-fi.com"], labels: ["ko-fi"], actionLabel: "Ko-fi" },
    { key: "instagram", hosts: ["instagram.com"], labels: ["instagram"], actionLabel: "Instagram" },
    { key: "fdroid", hosts: ["f-droid.org"], labels: ["f-droid"], actionLabel: "F-Droid" },
    { key: "apkpure", hosts: ["apkpure.com"], labels: ["apkpure"], actionLabel: "APKPure" },
    { key: "uptodown", hosts: ["uptodown.com"], labels: ["uptodown"], actionLabel: "Uptodown" },
    { key: "onestore", hosts: ["onestore.net"], labels: ["one store"], actionLabel: "ONE store" },
    { key: "openapk", hosts: ["openapk.net"], labels: ["openapk"], actionLabel: "OpenAPK" }
  ];

  const ACTION_SELECTOR = [
    "a[data-brand]", "a[data-update-link]", "a.text-link", "a.project-action", "a.section-link",
  ...
  const createIcon = (platform) => {
    ...
    if (pathData) {
      ...
      const monograms = { nexus: "N", apkpure: "A", uptodown: "U", onestore: "", openapk: "" };
      icon.classList.add("external-platform-icon--monogram");
      icon.textContent = Object.prototype.hasOwnProperty.call(monograms, platform.key) ? monograms[platform.key] : "•";
    }
    return icon;
  };
  ...
  const updateLabelFor = (link, platform) => link.hasAttribute("data-update-link") ? platform.actionLabel : "";

  const decorate = (link) => {
    const platform = platformFor(link);
    if (!platform) return;

    const updateLabel = updateLabelFor(link, platform);
    const existingIcon = link.querySelector(":scope > .external-platform-icon");
    if (existingIcon) {
      if (updateLabel) {
        let label = link.querySelector(":scope > .external-platform-label");
        if (!label) {
          label = document.createElement("span");
          label.className = "external-platform-label";
          link.append(label);
        }
        if (label.textContent !== updateLabel) label.textContent = updateLabel;
    ...
    const currentLabel = cleanLabel(link.textContent || "");
    const visibleLabel = updateLabel || currentLabel;
    if (!visibleLabel) return;

    const label = document.createElement("span");
    label.className = "external-platform-label";
    if (updateLabel) {
      label.textContent = updateLabel;
